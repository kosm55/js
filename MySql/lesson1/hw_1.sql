...
select avg(price) as avgPriceModel, model from cars group by model;

# підрахувати кількість кожної марки
select count(*) as contModel, model from cars group by model;

# знайти марку машин кількість яких найбільше
select count(*) as contModel, model from cars group by model order by contModel desc limit 1;
...
select * from cars where price>(select avg(price) as avgPriceAll from cars);

-- check
# select avg(price) as avgPriceAll from cars;
# select max(price) ,model from cars group by model order by max(price) desc ;