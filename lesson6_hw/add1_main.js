...
//     // let upper= repl.toUpperCase()
//     // return upper
//     //or short
//     let rez= str.replaceAll(' ', '-').toUpperCase()
//     return rez
// }
//  document.writeln(insert_dash(str)); // 'HTML-JAVASCRIPT-PHP'
...
// function upper(str){
//     let rez= str.slice(0,1).toUpperCase().concat(str.slice(1))
//     return rez
// }
// console.log(upper('asdff'))
...
// let n3 = 'Hermione__Granger'
//  function change(str){
//     let rez= str.replaceAll('..', ' ').replaceAll('---',' ').replaceAll('__', ' ')
//  return rez
// }
...
function search (str){
    let rezDog= str.indexOf('@',0)
    let rezDot= str.indexOf('.',0)
    if ((rezDog>0) && (rezDot>rezDog+3)){
        console.log('ok mail')
    }else{
        console.log('fail')
    }
}
search('someeMAIL@i.ua')
...
function cutString(str, n){
    let rez= str.split(' ')
    let newrez=[]
    for (let i = 0; i < newrez.length; i++) {
        const newrezElement = newrez[i];
        
    }
}

document.write(cutString(str, 5)) // 'Сила тяжести приложена к центру'
